...
We will describe the developing process for each parallelized method, including the challenges and the notions we made during development.
### 3.1 OpenMP
...
### 3.3 CUDA
From our observation, there are two part we need to do the parallelization which first part is $\mathbf{compute}$ function and the other is $\mathbf{update}$ part.
