...
//
{
  ...
  //***********************************************for convenience
  //begin of for loop
  vector<double> timeStorage;
  vector<string> atomName = {"atom/num","Hydrogen","Helium","Lithium","Carbon","Oxygen"};
  vector<double> atomMass = {1.008,4.003,6.941,12.011,15.999};
  vector<int> atomNumber = {500,600,700,800,900,1000};
  double timeAve = 0;
  for(int theNum=0;theNum<atomNumber.size();theNum++){
      np = atomNumber[theNum];
  for(int theMass=0;theMass<atomMass.size();theMass++){
...
//
  ...
  //end of the loop
      ...
      timeAve = timeAve/10;
      timeStorage.push_back(timeAve);
  }
  }
  //for loop print out runtime
  for(int i = 0;i<atomName.size();i++){
      cout<<left<<setw(10)<<atomName[i];
  }
  cout<<endl;
  for(int j = 0;j<atomNumber.size();j++){
      cout<<left<<setw(10)<<atomNumber[j];
        for(int i = 0;i<atomMass.size();i++){
            cout<<left<<setw(10)<<timeStorage[j*atomMass.size()+i];
        }
        cout<<endl;
  }
  //***********************************************
